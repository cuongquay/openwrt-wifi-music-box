...
(function() {
	...
	genParamStr = function(params, excludes) {
		var clientParams = genClientParams(params, excludes),
		    parts = [],
		    p;
	...
	isEJSObject = function(obj) {
		return (isObject(obj) && has(obj, '_type') && has(obj, '_self') && has(obj, 'toString'));
	};
	...
	ejs.TermStatsFacet = function(name) {
		...
		return {
			...
			order : function(o) {
				...
				o = o.toLowerCase();
				if (o === 'count' || o === 'term' || o === 'reverse_count' || o === 'reverse_term' || o === 'total' || o === 'reverse_total' || o === 'min' || o === 'reverse_min' || o === 'max' || o === 'reverse_max' || o === 'mean' || o === 'reverse_mean') {
	...
	ejs.TermsFacet = function(name) {
		...
		return {
			...
			order : function(o) {
				...
				o = o.toLowerCase();
				if (o === 'count' || o === 'term' || o === 'reverse_count' || o === 'reverse_term') {
	...
	ejs.TermsFilter = function(field, terms) {
		...
		return {
			...
			execution : function(e) {
				...
				e = e.toLowerCase();
				if (e === 'plain' || e === 'bool' || e === 'bool_nocache' || e === 'and' || e === 'and_nocache' || e === 'or' || e === 'or_nocache') {
	...
	ejs.Document = function(index, type, id) {
		...
		return {
			...
			doGet : function(successcb, errorcb) {
				...
				return ejs.client.get(url, genClientParams(params, paramExcludes), successcb, errorcb);
			},
	...
	ejs.FieldQuery = function(field, qstr) {
		...
		return {
			...
			fuzzyRewrite : function(m) {
				...
				m = m.toLowerCase();
				if (m === 'constant_score_auto' || m === 'scoring_boolean' || m === 'constant_score_boolean' || m === 'constant_score_filter' || m.indexOf('top_terms_boost_') === 0 || m.indexOf('top_terms_') === 0) {
	...
	ejs.FilteredQuery = function(someQuery, someFilter) {
		...
		return {
			...
			strategy : function(strategy) {
				...
				strategy = strategy.toLowerCase();
				if (strategy === 'query_first' || strategy === 'random_access_always' || strategy === 'leap_frog' || strategy === 'leap_frog_filter_first' || strategy.indexOf('random_access_') === 0) {
	...
	/**
	 @class
	 A <code>ObjectQuery</code> is a type of <code>Query</code> that accepts
	 text/numerics/dates, analyzes it, generates a query based on the
	 <code>ObjectQuery</code> type.

	 @name ejs.ObjectQuery

	 @desc
	 A Query that appects text, analyzes it, generates internal query based
	 on the ObjectQuery type.

	 @param {Object} field the document object query to query against
	 */
	ejs.ObjectQuery = function(oQuery) {

		/**
		 The internal query object. <code>Use get()</code>
		 @member ejs.ObjectQuery
		 @property {Object} query
		 */
		var query = oQuery;
	...
	ejs.NestedQuery = function(path) {
		...
		return {
			...
			scoreMode : function(mode) {
				...
				if (mode === 'avg' || mode === 'total' || mode === 'max' || mode === 'none' || mode === 'sum') {
	...
	ejs.MultiSearchRequest = function(conf) {
		...
		return {
			...
			searchType : function(t) {
				...
				t = t.toLowerCase();
				if (t === 'dfs_query_then_fetch' || t === 'dfs_query_and_fetch' || t === 'query_then_fetch' || t === 'query_and_fetch' || t === 'scan' || t === 'count') {
	...
	ejs.Rescore = function(windowSize, qry) {
		...
		return {
			...
			scoreMode : function(s) {
				...
				s = s.toLowerCase();
				if (s === 'total' || s === 'min' || s === 'max' || s === 'multiply' || s === 'avg') {
	...
	ejs.Shape = function(type, coords) {
		...
		var shape = {},
			...
			var valid = false;
			if (t === 'point' || t === 'linestring' || t === 'polygon' || t === 'multipoint' || t === 'envelope' || t === 'multipolygon' || t === 'circle' || t === 'multilinestring') {
				valid = true;
			}
	...
	ejs.Sort = function(fieldName) {
		...
		var sort = {},
		    key =
		    fieldName, // defaults to field search
		    geo_key = '_geo_distance', // used when doing geo distance sort
		    script_key = '_script';
		// used when doing script sort
	...
	ejs.DirectSettingsMixin = function() {
		...
		return {
			...
			stringDistance : function(s) {
				...
				s = s.toLowerCase();
				if (s === 'internal' || s === 'damerau_levenshtein' || s === 'levenstein' || s === 'jarowinkler' || s === 'ngram') {
	...
	ejs.GeoDistanceAggregation = function(name) {
		...
		return extend(_common, {
			...
			unit : function(unit) {
				...
				if (unit === 'in' || unit === 'yd' || unit === 'ft' || unit === 'km' || unit === 'NM' || unit === 'mm' || unit === 'cm' || unit === 'mi' || unit === 'm') {
			...
			distanceType : function(type) {
				...
				type = type.toLowerCase();
				if (type === 'plane' || type === 'arc' || type === 'sloppy_arc' || type === 'factor') {
	...
	ejs.TermsAggregation = function(name) {
		...
		return extend(_common, {
			...
			valueType : function(v) {
				...
				v = v.toLowerCase();
				if (v === 'string' || v === 'double' || v === 'float' || v === 'long' || v === 'integer' || v === 'short' || v === 'byte') {
	...
	ejs.TopHitsAggregation = function(name) {
		...
		var _common = ejs.MetricsAggregationMixin(name, 'top_hits'),
		    agg = _common.toJSON();
