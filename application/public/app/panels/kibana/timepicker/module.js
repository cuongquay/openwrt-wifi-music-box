 ...
 */
define(['angular', 'app', 'lodash', 'moment', 'kbn'], function(angular, app, _, moment, kbn) {
	'use strict';

	var module = angular.module('go4smac.panels.timepicker', []);
	app.useModule(module);

	module.controller('timepicker', ["$scope", "$rootScope", "$modal", "$q", "filterSrv",
	function($scope, $rootScope, $modal, $q, filterSrv) {
		$scope.panelMeta = {
			status : "Stable",
			description : "A panel for controlling the time range filters. If you have time based data, " + " or if you're using time stamped indices, you need one of these"
		};

		// Set and populate defaults
		var _d = {
			status : "Stable",
			time_options : ["24h", "2d", "7d", "30d", "60d", "90d", "1y", "5y"],
			intervals : ["3h", "6h", "12h", "2d", "1w", "1w", "1M", "1y"],
			timefield : '@timestamp'
		};
		...
		$scope.getTimeAgo = function(timespan) {
			return moment.utc(kbn.time_ago(timespan)).fromNow();
		};
		...
		$scope.setAbsoluteTimeFilter = function(time) {
			...
			// Fix interval of one day for range selection
			filterSrv.set_interval("1d");
		...
		$scope.setRelativeFilter = function(timespan) {
			...
			filterSrv.set_interval($scope.panel.intervals[$scope.panel.time_options.indexOf(timespan)]);
			// Clear all time filters, set a new one
			filterSrv.removeByType('time', true);
