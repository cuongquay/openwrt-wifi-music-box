...
define([
  ...
  var module = angular.module('go4smac.panels.query', []);
  ...
  module.controller('query', function($scope, querySrv, $rootScope, dashboard, $q, $modal) {
    ...
    var queryHelpModal = $modal({
      template: './app/panels/kibana/query/helpModal.html',
      persist: true,
      show: false,
      scope: $scope,
    });
    ...
    $scope.queryConfig = function(type) {
      return "./app/panels/kibana/query/editors/"+(type||'lucene')+".html";
    };

    $scope.queryHelpPath = function(type) {
      return "./app/panels/kibana/query/help/"+(type||'lucene')+".html";
    };
