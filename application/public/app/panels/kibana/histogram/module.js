 ...
 *
 */
define(['angular', 'app', 'jquery', 'lodash', 'kbn', 'moment', './timeSeries', 'numeral', 'jquery.flot', 'jquery.flot.events', 'jquery.flot.selection', 'jquery.flot.time', 'jquery.flot.byte', 'jquery.flot.stack', 'jquery.flot.stackpercent', 'jquery.flot.threshold'], function(angular, app, $, _, kbn, moment, timeSeries, numeral) {

	'use strict';

	var module = angular.module('go4smac.panels.histogram', []);
	app.useModule(module);

	module.controller('histogram', ["$rootScope", "$scope", "querySrv", "dashboard", "filterSrv",
	function($rootScope, $scope, querySrv, dashboard, filterSrv) {
		$scope.panelMeta = {
	      modals : [
	        {
	          description: "Inspect",
		...
		$scope.get_data = function(data, segment, query_id) {
			try {
				...
				if (request && Object.keys(request).length > 0) {
					if ((!$scope.panel.annotate.enable || $scope.panel.mode === 'cardinality') && request.searchType) {
					...
					// Build the query
		            if ($scope.panel.mode === 'cardinality') {
		                var boolQuery = $scope.ejs.BoolQuery();
		                var query = $scope.ejs.FilteredQuery(boolQuery, filterSrv.getBoolFilter(filterSrv.ids())),
		                        agAggs = $scope.ejs.TermsAggregation('terms').field($scope.panel.field),
		                        daAggs = $scope.ejs.DateHistogramAggregation('date_histogram').field($scope.panel.time_field).interval(_interval),
		                        caAggs = $scope.ejs.CardinalityAggregation('cardinality').field($scope.panel.cardinality_field);
		                
		                if($scope.panel.precision_threshold) {
		                    caAggs.precisionThreshold($scope.panel.precision_threshold);
		                }
		
		                _.each(queries, function(q) {
		                    boolQuery = boolQuery.should(querySrv.toEjsObj(q));
		                    var fiAggs = $scope.ejs.FilterAggregation(q.id).filter($scope.ejs.QueryFilter(querySrv.toEjsObj(q)));
		                    request = request.aggs(fiAggs.agg(agAggs.agg(daAggs.agg(caAggs))));
		                });
		
		                request = request.query(query);
		                request = request.aggs(agAggs.agg(daAggs.agg(caAggs)));
		            } else {
		                _.each(queries, function(q) {
		                    ...
		                    request = request.facet(facet).size($scope.panel.annotate.enable ? $scope.panel.annotate.size : 0);
		                });
		            }
		
					if ($scope.panel.annotate.enable) {
						var query = $scope.ejs.FilteredQuery($scope.ejs.QueryStringQuery($scope.panel.annotate.query || '*'), filterSrv.getBoolFilter(filterSrv.idsByType('time')));
						request = request.query(query);
						...
						request = request.sort([$scope.ejs.Sort($scope.panel.annotate.sort[0]).order($scope.panel.annotate.sort[1]).ignoreUnmapped(true), $scope.ejs.Sort($scope.panel.time_field).desc().ignoreUnmapped(true)]);
					}
					...
					return results.then(function(results) {
						...
						if (segment === 0) {
		                    ...
		                    if ($scope.panel.mode === 'cardinality') {
		                        var totalRequest = $scope.ejs.Request().indices(dashboard.indices).searchType('count');
		                        var boolQuery = $scope.ejs.BoolQuery();
		                        var query = $scope.ejs.FilteredQuery(boolQuery, filterSrv.getBoolFilter(filterSrv.ids())),
		                            agAggs = $scope.ejs.TermsAggregation('terms').field($scope.panel.field),
		                            caAggs = $scope.ejs.CardinalityAggregation('cardinality').field($scope.panel.cardinality_field);
		                        ...
		                        _.each(queries, function(q) {
		                            boolQuery = boolQuery.should(querySrv.toEjsObj(q));
		                            var fiAggs = $scope.ejs.FilterAggregation(q.id).filter($scope.ejs.QueryFilter(querySrv.toEjsObj(q)));
		                            totalRequest.aggs(fiAggs.agg(agAggs.agg(caAggs)));
		                        });
		                        totalRequest = totalRequest.query(query);
		                    ...
		                    if ($scope.panel.mode === 'cardinality') {
		                        if (!(_.isUndefined(results.error))) {                        	
		                            ...
		                            _.each(queries, function(q, k) {
		                                $scope.legend[k] = {
		                                    query: q,
		                                    alias: $scope.panel.queries.aliases[q.id] || q.alias,
		                                };
		                                if (!results.aggregations || !results.aggregations[q.id]) return false;
		                                var tmpTerms = results.aggregations[q.id];
		                                _.each(tmpTerms.terms.buckets, function(termItem) {
		                                        var counters = {}, time_series;
		                                        _.each(termItem.date_histogram.buckets, function(dateItem) {
		                                            counters[dateItem['key']] = dateItem.cardinality.value;
		                                        });
		                                        ...
		                                        _.each(termItem.date_histogram.buckets, function(entry) {
		                                            var value = (time_series._data[entry.key] || 0) + entry.doc_count;
		                                            time_series.addValue(entry.key, value);
		                                        });
		
		                                        data[i] = {counters: counters,
		                                            color: q.color,
		                                            label: $scope.panel.queries.aliases[query_id],
		                                            time_series: time_series
		                                        };                                        
		                                        data[i].info = _.extend(q);                                    
		                                });
		                                i++;
		                            });
		                        }
		                    } else {
		                        var i = 0, time_series, hits, counters;
		                        ...
		                        _.each(queries, function(q) {
		                            ...
		                            _.each(query_results.entries, function(entry) {
		                                ...
		                                if ($scope.panel.mode === 'count') {
		                                    ...
		                                    value = (((time_series._data[entry.time] || 0) * (counters[entry.time] - entry.count)) + entry.mean * entry.count) / (counters[entry.time]);
							...
							if ($scope.panel.annotate.enable) {
								$scope.annotations = $scope.annotations.concat(_.map(results.hits.hits, function(hit) {
									...
									return {
										...
										title : null,
										description : "<small><i class='icon-tag icon-flip-vertical'></i> " + _h[$scope.panel.annotate.field] + "</small><br>" + moment(hit.sort[1]).format('YYYY-MM-DD HH:mm:ss'),
										score : hit.sort[0]
									};
								}));
								// Sort the data
						...
						if (segment < dashboard.indices.length - 1) {
		                    ...
		                    if ($scope.panel.mode === 'cardinality') {
		                        $scope.totalRequest.doSearch().then(function(result){
		                            if (result.aggregations) {
		                                var rs = _.clone(result.aggregations);
		                                _.each(queries, function(q) {
		                                    var tmp = ((rs[q.id].terms.buckets && rs[q.id].terms.buckets[0]
		                                                && rs[q.id].terms.buckets[0].cardinality
		                                                && rs[q.id].terms.buckets[0].cardinality.value) || 0);
		                                    data[q.id] = tmp;
		                                    $scope.hits += tmp;
		                                    $scope.legend[q.id].hits = tmp;
		                                });
		                                $scope.$emit('render', data);
		                            }
		                        });
					...
					try {
						var options = {
							...
							series : {
								...
								thresholds: scope.panel.thresholds || null,
								lines : {
							...
							grid : {
								...
								hoverable : true,
								color : '#c8c8c8',
								markings: scope.panel.markings || null
							}
						};
				...
				elem.bind("plothover", function(event, pos, item) {
					...
					if (item) {
						if (item.series.info.alias || scope.panel.tooltip.query_as_alias) {
							group = '<small style="font-size:0.9em;">' + '<i class="icon-circle" style="color:' + item.series.color + ';"></i>' + ' ' + (item.series.info.alias || item.series.info.query) + '</small><br>';
						} else {
							group = kbn.query_color_dot(item.series.color, 15) + ' ';
						}
						value = (scope.panel.stack && scope.panel.tooltip.value_type === 'individual') ? item.datapoint[1] - item.datapoint[2] : item.datapoint[1];
						if (scope.panel.y_format === 'bytes') {
						...
						timestamp = scope.panel.timezone === 'browser' ? moment(item.datapoint[0]).format('YYYY-MM-DD HH:mm:ss') : moment.utc(item.datapoint[0]).format('YYYY-MM-DD HH:mm:ss');
						$tooltip.html(group + value + interval + " @ " + timestamp).place_tt(pos.pageX, pos.pageY);
					} else {
						$tooltip.detach();
					}
				});
