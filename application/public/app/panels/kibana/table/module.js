 ...
 *
 */
define(['angular', 'app', 'lodash', 'kbn', 'moment', 'jsonpath'], function(angular, app, _, kbn, moment) {
	'use strict';

	var module = angular.module('go4smac.panels.table', []);
	...
	module.controller('table', function($rootScope, $scope, $modal, $q, $compile, $timeout, fields, querySrv, dashboard, filterSrv) {
		$scope.panelMeta = {
			...
			editorTabs : [{
				title : 'Paging',
				src : 'app/panels/kibana/table/pagination.html'
			}, {
				title : 'Queries',
				src : 'app/partials/querySelect.html'
			}],
			status : "Stable",
			description : "A paginated table of records matching your query or queries. Click on a row to " + "expand it and review all of the fields associated with that document. <p>"
		};
		...
		$scope.termsModal = function(field, chart) {
			...
			$timeout(function() {
				...
				showModal(angular.toJson($scope.adhocOpts), 'terms');
			}, 0);
		};
		...
		$scope.statsModal = function(field) {
			...
			$timeout(function() {
				...
				showModal(angular.toJson($scope.adhocOpts), 'stats');
			}, 0);
		};
		...
		$scope.onPageNext = function() {
			if ($scope.data.length > $scope.panel.offset + $scope.panel.size) {
				$scope.panel.offset = ($scope.panel.offset + $scope.panel.size);	
			}			
		};
		$scope.onPageBack = function() {
			if ($scope.panel.offset > 0) {
				$scope.panel.offset = ($scope.panel.offset - $scope.panel.size);	
			}
		};
		...
		$scope.toggle_micropanel = function(field, groups) {
			...
			var nodeInfo = $scope.ejs.client.get('/' + dashboard.indices + '/_mapping/field/' + field, undefined, undefined, function(data, status) {
				console.log(status);
				return;
			});
		...
		$scope.get_data = function(segment, query_id) {
			try {
				var _segment, request, boolQuery, queries, sort;
				...
				if (request && Object.keys(request).length > 0) {
					...
					request = request.query($scope.ejs.FilteredQuery(boolQuery, filterSrv.getBoolFilter(filterSrv.ids()))).highlight($scope.ejs.Highlight($scope.panel.highlight).fragmentSize(2147483647)// Max size of a 32bit unsigned int
					.preTags('@start-highlight@').postTags('@end-highlight@')).size($scope.panel.size * $scope.panel.pages).sort(sort);
					...
					request.doSearch().then(function(results) {
						...
						if ($scope.query_id === query_id) {
							...
							$scope.data = $scope.data.concat(_.map(results.hits.hits, function(hit) {
								var _h = _.clone(hit), _p = _.omit(hit, '_source', 'sort', '_score');
						...
						if (($scope.data.length < $scope.panel.size * $scope.panel.pages || !((_.contains(filterSrv.timeField(), $scope.panel.sort[0])) && $scope.panel.sort[1] === 'desc')) && _segment + 1 < dashboard.indices.length) {
		...
		$scope.exportData = function() {
			...
			$scope.data.forEach(function(item) {
				...
				angular.extend(data, item._source);
				exportData.push(data);
			});
			alasql('SELECT * INTO XLSX("' + $scope.panel.title + '.xlsx", {headers:true}) FROM ?', [exportData]);
		};
