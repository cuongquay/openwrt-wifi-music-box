define([
   ...
   'd3'
        ...
        function(angular, app, config, require, d3) {
           ...
           var module = angular.module('kibana.panels.devices', []);
           app.useModule(module);
           module.controller('devicesCtrl', ['$scope', 'alertSrv', 'ejsResource', 'querySrv', function($scope, alertSrv, ejsResource, querySrv) {
                 function search(options) {
                    var date = new Date();
                    $scope.get_devices('log-' + date.getFullYear() + '.*');
                 }
                 $scope.init = function() {
                    var userEmail = 'guest';
                 ...
                 $scope.get_devices = function(indices, options) {
                    ...
                    var aggs = $scope.ejs.TermsAggregation('device')
                            .field('device')
                            .order('_count', 'desc');

                    $scope.request.aggs(aggs);
                    $scope.request.doSearch(function(data) {
                       var device_data = data.aggregations.device.buckets;
                       var total = 0;
                       for (var i = 0; i < device_data.length; i++)
                          total += device_data[i].doc_count;
                       $scope.root = {key: 'All Devices', doc_count: total, version: {buckets: device_data}};
                       ...
                       $scope.$on('tab_select', function(e, panel) {
                          if (panel.panels) {
                             for (var i = 0; i < panel.panels.length; i++) {
                                if (panel.panels[i].type == 'devices') {
                                   $scope.draw_chart($scope.root, $scope.tooltip);
                                   break;
                                }
                             }
                 ...
                 $scope.draw_chart = function(root, tooltip) {
                    d3.select('#device-chart').html("");
                    // d3 start here
                    var width = d3.select('#device-chart').style('width').replace('px', '');
                    ...
                    var svg = d3.select("#device-chart").append("svg")
                            .attr("width", width)
                            .attr("height", height)
                            .append("g")
                            .attr("transform", "translate(" + width / 2 + "," + (height / 2 + 10) + ")");

                    var inner = d3.select('#device-chart').append('div')
                            .attr('class', 'chart-summary').append('div')
                            .attr('class', 'item').append('div')
                            .attr('class', 'inner');

                    inner.append('div').attr('class', 'title').text('Top Devices');
                    var bar = inner.append('div').attr('class', 'bar');
                    var d = $scope.root.version.buckets[0];
                    var top = d.key == 'Other' ? 'PC/Laptop' : d.key;
                    ...
                    bar.selectAll('div')
                            ...
                            .on('mouseover', function(d) {
                               number.html((d.key == 'Other' ? 'PC/Laptop' : d.key) + '<br/>' + d.doc_count + ' / ' + $scope.root.doc_count + '<br/>' + (d.doc_count * 100 / $scope.root.doc_count).toFixed(2) + '%');
                            })
                    ...
                    function show(d) {
                       tooltip.html(function() {
                          return (d.key == 'Other' ? 'PC/Laptop' : d.key) + ' (' + d.doc_count + ')';
                       })
