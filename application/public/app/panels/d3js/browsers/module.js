define([
   ...
   'd3'
        ...
        function(angular, app, config, require, d3) {
           ...
           var module = angular.module('kibana.panels.browsers', []);
           app.useModule(module);
           module.controller('browsersCtrl', ['$scope', 'alertSrv', 'ejsResource', 'querySrv', function($scope, alertSrv, ejsResource, querySrv) {
                 function search(options) {
                    var date = new Date();
                    $scope.get_browsers('log-' + date.getFullYear() + '.*');
                 }
                 $scope.init = function() {
                    var userEmail = 'guest';
                 ...
                 $scope.get_browsers = function(indices, options) {
                    ...
                    var aggs = $scope.ejs.TermsAggregation('browser')
                            .field('browser')
                            .order('_count', 'desc');
                    ...
                    $scope.request.doSearch(function(data) {
                       var browsers_data = data.aggregations.browser.buckets;
                       var total = 0;
                       for (var i = 0; i < browsers_data.length; i++)
                          total += browsers_data[i].doc_count;
                       $scope.root = {key: 'All Browsers', doc_count: total, version: {buckets: browsers_data}};
                       ...
                       $scope.$on('tab_select', function(e, panel) {
                          if (panel.panels) {
                             for (var i = 0; i < panel.panels.length; i++) {
                                if (panel.panels[i].type == 'browsers') {
                                   $scope.draw_chart($scope.root, $scope.tooltip);
                                   break;
                                }
                             }
                          }
                          if (panel.type == 'browsers') {
                             $scope.draw_chart($scope.root, $scope.tooltip);
                          }
                       });
...
//                       console.log(error);
                 ...
                 $scope.draw_chart = function(root, tooltip) {
                    d3.select('#browser-chart').html("");
                    // d3 start here
                    var width = d3.select('#browser-chart').style('width').replace('px', '');
                    ...
                    var svg = d3.select("#browser-chart").append("svg")
                            .attr("width", width)
                            .attr("height", height)
                            .append("g")
                            .attr("transform", "translate(" + width / 2 + "," + (height / 2 + 10) + ")");

                    var inner = d3.select('#browser-chart').append('div')
                            .attr('class', 'chart-summary').append('div')
                            .attr('class', 'item').append('div')
                            .attr('class', 'inner');

                    inner.append('div').attr('class', 'title').text('Top Browsers');
