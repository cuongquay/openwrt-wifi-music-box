define([
  ...
  var module = angular.module('go4smac.filters');
  ...
  module.filter('pinnedQuery', [ 'querySrv',  function(querySrv) {
  ...
  module.filter('pinnedFilter', [ 'filterSrv',  function(filterSrv) {
    return function( items, pinned) {
      var ret = _.filter(filterSrv.ids(),function(id){
        var v = filterSrv.list()[id];
        if(!_.isUndefined(v.pin) && v.pin === true && pinned === true) {
          return true;
        }
        if((_.isUndefined(v.pin) || v.pin === false) && pinned === false) {
  ...
  module.filter('word_break', [ function() {
    return function(content, number, append_str) {
        var aWords = content.split(' ');
        if( aWords.length > number ) {
          aWords = aWords.slice(0, number);
          content = aWords.join(' ');
          if(append_str){
