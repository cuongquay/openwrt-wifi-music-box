define(['angular', 'lodash', 'config', 'kbn'], function(angular, _, config, kbn) {
	'use strict';
	...
	module.service('querySrv', function(dashboard, ejsResource, filterSrv, esVersion, $q) {
		...
		this.colors = ["#7EB26D", "#EAB839", "#6ED0E0", "#EF843C", "#E24D42", "#1F78C1", "#BA43A9", "#705DA0", //1
		...
		this.queryTypes = {
			...
			topN : {
				...
				resolve : function(q) {
					...
					// Terms mode
					request = request.facet(ejs.TermsFacet('query').field(q.field).size(q.size).facetFilter(ejs.QueryFilter(ejs.FilteredQuery(ejs.QueryStringQuery(q.query || '*'), filterSrv.getBoolFilter(filterSrv.ids()))))).size(0);
					...
					return results.then(function(data) {
						...
						var i = -1;
						return _.map(data.facets.query.terms, function(t) {++i;
		...
		this.init = function() {			
			dashboard.current.services.query.ids = _.intersection(_.map(dashboard.current.services.query.list, function(v, k) {
		...
		this.toEjsObj = function(q) {
			...
			case 'lucene':
				if ( typeof (q.query) === "string")
					...
					return ejs.ObjectQuery(q.query);
			case 'regex':
		...
		this.getQueryObjs = function(ids) {
			if (_.isUndefined(ids)) {
				...
				return _.flatten(_.map(ids, function(id) {
					return _.where(resolvedQueries, {
						parent : id
					});
				}));
			}
		...
		this.idsByMode = function(config) {
			switch(config.mode) {
			case 'all':
				return _.pluck(_.where(dashboard.current.services.query.list, {
					enable : true
				}), 'id');
			case 'pinned':
				return _.pluck(_.where(dashboard.current.services.query.list, {
			...
			case 'selected':
				return _.intersection(_.pluck(_.where(dashboard.current.services.query.list, {
					enable : true
				}), 'id'), config.ids);
			default:
				return _.pluck(_.where(dashboard.current.services.query.list, {
		...
		this.resolve = function() {
			...
			return $q.all(_.map(dashboard.current.services.query.ids, function(q) {
				return self.queryTypes[dashboard.current.services.query.list[q].type].resolve(_.clone(dashboard.current.services.query.list[q])).then(function(data) {
					return data;
				});
			})).then(function(data) {
