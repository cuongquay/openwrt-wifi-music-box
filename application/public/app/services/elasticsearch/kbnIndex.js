define(['angular', 'lodash', 'config', 'moment'], function(angular, _, config, moment) {
	'use strict';
	...
	module.service('kbnIndex', function($http, alertSrv, ejsResource) {
		...
		function resolve_indices(indices) {
			...
			something = ejs.client.get("/" + indices.join(",") + "/_aliases?ignore_unavailable=true&ignore_missing=true", undefined, undefined, function(data, p) {
				if (p === 404) {
					return [];
				} else {
					try {
						if (p === 0) {
							alertSrv.set('Error', "Could not contact Elasticsearch at " + ejs.config.server + ". Please ensure that Elasticsearch is reachable from your system.", 'error');
						} else {
							alertSrv.set('Error', "Could not reach " + ejs.config.server + "/_aliases. If you" + " are using a proxy, ensure it is configured correctly", 'error');
						}
					} catch (ex) {
						console.error(ex);
					}
