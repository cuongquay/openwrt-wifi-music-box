define(['angular', 'lodash', 'config', 'kbn'], function(angular, _, config, kbn) {
	...
	module.service('filterSrv', function(dashboard, ejsResource, $rootScope, $timeout) {
		...
		this.set_interval = function(val) {
			dashboard.current.services.filter.interval = val;
		};
		this.get_interval = function() {
			return dashboard.current.services.filter.interval;
		};
		...
		this.set = function(filter, id, noRefresh) {
			...
			dashboard.current.services.filter.ids = dashboard.current.services.filter.ids = _.intersection(_.map(dashboard.current.services.filter.list, function(v, k) {
				return parseInt(k, 10);
			}), dashboard.current.services.filter.ids);
		...
		this.toEjsObj = function(filter) {
			...
			case 'time':
				...
				return _f;
			case 'range':
				return ejs.RangeFilter(filter.field).from(filter.from).to(filter.to);
			case 'querystring':
			...
			case 'pin':
				if (_.where(filter.value, {
						enable : true
					}).length === 0)
					return false;
				return ejs.TermsFilter(filter.field, _.pluck(_.where(filter.value, {
					enable : true
				}), 'value'));
			case 'exists':
		...
		this.timeRange = function(parse) {
			var _t = _.last(_.where(dashboard.current.services.filter.list, {
				type : 'time',
				active : true
			}));
			if (_.isUndefined(_t)) {
